...
- **112 unit tests** — a dependency-free test runner covers the core logic (run `node tests/app.test.js`)
...
- `adjustFilterIfNeeded` — auto-switches to 'all' filter when current filter is empty (6 tests)
...
├── tests/
│   └── app.test.js         # Dependency-free unit test suite (112 tests)
├── discussion/
